...
async function completeRouteJob(){
 ...
 try{
  ...
  setTimeout(()=>{
   ...
   activeRouteJob=null;
   go('home',document.querySelector('[data-page="home"]'));
   renderEarnings();
  },3000);
 }catch(e){
...
function openBillForJob(j){go('bills',document.querySelector('[data-page="bills"]'));resetBill(false);const g=id=>document.getElementById(id);if(!g('name'))return;g('name').value=first(j,['costumername','customerName','customer'],'');g('phone').value=first(j,['costumerphone','customerPhone','phone'],'');g('address').value=first(j,['costumerAddress','customerAddress','address'],'');g('service').value=first(j,['service','serviceType'],'');g('invoice').value='DHS-'+Date.now().toString().slice(-6);g('date').value=new Date().toISOString().slice(0,10);generate(false);window.scrollTo({top:0,behavior:'smooth'});toast('Bill page opened for '+(g('name').value||'customer'))}
